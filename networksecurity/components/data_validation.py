...
class DataValidation:
    ...
    def initiate_data_validation(self)-> DataValidationArtifact:
        try:
            ...
            ##reading the data from train and test and validate the number of columns
            train_dataframe = DataValidation.read_data(train_file_path)
            test_dataframe = DataValidation.read_data(test_file_path)

            status= self.validate_no_of_columns(dataframe=train_dataframe)
            if not status:
                error_message="Train data frame does not contain all the columns"
                logging.info(f"{error_message}")

            status= self.validate_no_of_columns(dataframe=test_dataframe)
            if not status:
                error_message="Test data frame does not contain all the columns"
                logging.info(f"{error_message}")
